...
async function materialList(company,division){
  const response = await axios.get(process.env.backend_url+`/api/materials?filters[company][id][$eq]=${company}&filters[division][id][$eq]=${division}&populate=participants,division,Images`).catch((e)=>{
